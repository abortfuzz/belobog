...
pub fn generate_stackless_bytecode<'a>(
...
) -> Result<(Vec<ModuleBytecode>, GlobalEnv)> {
    let modules: Vec<&CompiledModule> = modules.into_iter().collect();
    if modules.is_empty() {
        return Err(anyhow!("no modules provided to stackless generator"));
    }
    ...
    for module_env in env.get_modules() {
        ...
        for func_env in module_env.get_functions() {
            ...
            if func_env.get_jump_tables().len() > 0 {
...
pub fn generate_stackless_bytecode_for_module<'a>(
...
) -> Result<(ModuleBytecode, GlobalEnv)> {
    ...
    all.0
        .pop()
        .map(|m| (m, all.1))
        .ok_or_else(|| anyhow!("stackless generator produced no modules"))
}
